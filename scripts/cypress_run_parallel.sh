...
# Clean up the state files between runs.
rm services/frontend/.cypress-fifo 2>/dev/null
rm services/frontend/.cypress-fifo-lock 2>/dev/null
rm services/frontend/.cypress-logs 2>/dev/null
rm services/frontend/.cypress-logs-lock 2>/dev/null

# Split the spec files.
cd services/frontend
npm run cypress:parallel:split
cd -
...
for i in $(seq $NUMBER_OF_INSTANCES); do
    tmux send-keys -t $((i - 1)) "cd services/frontend && npm run cypress:parallel:run $i" C-m

    # Need to wait between worker invocations so that the first one has time to setup the FIFO.
    sleep 1
done
